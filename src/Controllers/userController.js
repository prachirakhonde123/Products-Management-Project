...
const registerUser = async function (req, res) {
    try {
        ...
        //_______________________________Validation for ProfileImage______________________________________________
...
const updateuser = async function (req, res) {
    ...
    if (fname || lname || email || phone || password || address) {
        ...
        if (password) {
          ...
          //  var bcryptPassword = await bcrypt.hash(password, 10)
            updateuse["password"]= await bcrypt.hash(password, 10)
        }
        ...
        if (address) {
            if (address.shipping) {
                if (address.shipping.street) {
                    ...
                    updateuse["address.shipping.street"]= address.shipping.street
                }
                if (address.shipping.city) {
                    if (!address.shipping.city) {
                        return res.status(400).send({ status: false, message: "Shipping : City feild is Mandatory" })
                    ...
                    updateuse["address.shipping.city"]= address.shipping.city
                }
                if (address.shipping.pincode) {
                    if (!address.shipping.pincode) {
                        return res.status(400).send({ status: false, message: "Shipping : Pincode feild is Mandatory" })
                    ...
                    updateuse["address.shipping.pincode"]= address.shipping.pincode
                }
            ...
            if (address.billing) {
                if (address.billing.street) {
                    ...
                    updateuse["address.billing.street"]= address.billing.street
                }
                if (address.billing.city) {
                    if (!address.billing.city) {
                        return res.status(400).send({ status: false, message: "billing : City feild is Mandatory" })
                    ...
                    updateuse["address.billing.city"]= address.billing.city
                }
                ...
                if (address.billing.pincode) {
                    ...
                    updateuse["address.billing.pincode"]= address.billing.pincode
                }
             //   return res.status(400).send("addresss shipping is mandatory")
            }
        }
   ...
   // var uploadImage = await uploadFile.uploadFile(file[0]);
    updateuse["profileImage"]= await uploadFile.uploadFile(file[0]);
    }



    //====================================Updating Profile=======================================================================================
    const update = await userModel.findOneAndUpdate({ _id: user }, { $set: updateuse}, { new: true })
    if (!update) {
